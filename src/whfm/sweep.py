...
def _load_runtime_module(name: str):
    """Load a module from the installed WHFM package."""
...
class SweepConfig:
    ...
    def from_dict(cls, data: Mapping[str, Any], *, base_dir: Path) -> "SweepConfig":
        ...
        trainer = str(data.get("trainer", "v1"))
        if trainer not in {"v1", "v2"}:
            raise ValueError("trainer must be 'v1' or 'v2'.")
...
def build_effective_train_dict(
    ...
    *,
    ...
    if gpu == "cpu":
        ...
        effective["cuda_device"] = None
    else:
        effective["device"] = "cuda:0"
        effective["cuda_device"] = 0
...
def expand_jobs(config: SweepConfig, *, validate: bool = True) -> list[SweepJob]:
    ...
    trainer_v2_module = _load_runtime_module("trainer_v2") if validate and config.trainer == "v2" else None
    jobs: list[SweepJob] = []
    for problem_index, problem_config in enumerate(config.problem_configs):
        ...
        for seed_index, seed in enumerate(config.seeds):
            ...
            if validate:
                if config.trainer == "v2":
                    trainer_v2_module.TrainV2Config.from_dict(effective)
                else:
...
def run_training_job(
    ...
    problem_config_path: str | Path,
    *,
    trainer_name: str = "v1",
) -> Path:
    config_module = _load_runtime_module("config")
    if trainer_name == "v2":
        trainer_module = _load_runtime_module("trainer_v2")
        train_config = trainer_module.load_train_v2_config(train_config_path)
        trainer_cls = trainer_module.HamiltonianTrainerV2
    elif trainer_name == "v1":
        trainer_module = _load_runtime_module("trainer")
        train_config = config_module.load_train_config(train_config_path)
        trainer_cls = trainer_module.HamiltonianTrainer
    else:
        raise ValueError("trainer_name must be 'v1' or 'v2'.")
    problem_config = config_module.load_problem_config(problem_config_path)
    trainer = trainer_cls(train_config, problem_config)
    trainer.train()
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--trainer", choices=("v1", "v2"), default="v1", help=argparse.SUPPRESS)
    return parser
...
def main(argv=None) -> int:
    ...
    if args.run_job:
        ...
        run_training_job(args.run_job, args.problem_config, trainer_name=args.trainer)
        return 0
